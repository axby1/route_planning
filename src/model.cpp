...
//This function converts raw data from XML into structured types
static Model::Road::Type String2RoadType(std::string_view type) //string_view is a class which is a non-owning reference(ie; no new string created) to a string
{
...
Model::Model( const std::vector<std::byte> &xml ) //std::byte used for using raw/binary data type safely
{
    LoadData(xml);
...
//this function processes the XML data using pugiXML library and parses OpenStreetMap(map.osm) file formatted in XML
void Model::LoadData(const std::vector<std::byte> &xml)
{
    using namespace pugi;
    
    xml_document doc; //pugi object
    if( !doc.load_buffer(xml.data(), xml.size()) )
        throw std::logic_error("failed to parse the xml file");
    
    if( auto bounds = doc.select_nodes("/osm/bounds"); !bounds.empty() ) { //select nodes defining geo bounds and extract min & max lat,long from bounds 
    ...
    for( const auto &node: doc.select_nodes("/osm/node") ) {  //map each node id to its index in m_Nodes vector 
    ...
    for( const auto &way: doc.select_nodes("/osm/way") ) {
        ...
        for( auto child: node.children() ) {
            ...
            if( name == "nd" ) { //referencing node ids forming connections in the way
            ...
            else if( name == "tag" ) { //extracts attributes and store in corresponding vectors defined in model.h
    ...
    //processing relation and associated with multipolygons
...
//recursive function call to traverse new paths(DFS) 
static bool TrackRec(const std::vector<int> &open_ways,
                     const Model::Way *ways,
                     std::vector<bool> &used,
                     std::vector<int> &nodes) 
{
    if( nodes.empty() ) {			//exploring new ways
        ...
        if( head == tail && nodes.size() > 1 ) //indicates multiple paths ie; a cycle
            return true;
        for( int i = 0; i < open_ways.size(); ++i )
            if( !used[i] ) {
                const auto &way_nodes = ways[open_ways[i]].nodes;
...
//to find sequence of nodes(ie; set of open paths) using helper fn TrackRec
static std::vector<int> Track(std::vector<int> &open_ways, const Model::Way *ways)
{
    ...
    std::vector<int> nodes;    
    if( TrackRec(open_ways, ways, used, nodes) ) //recursive tracking for sequence of nodes
        for( int i = 0; i < open_ways.size(); ++i )
            if( used[i] )
                open_ways[i] = -1;
    return nodes; //vector storing nodes that form a path
}

//process outer and inner ways of multiploygon and identify and construct closed shapes
void Model::BuildRings( Multipolygon &mp )
{
    auto is_closed = []( const Model::Way &way ) {  //checks if way is closed by verifying if it has more than one node and(first==last) 
        return way.nodes.size() > 1 && way.nodes.front() == way.nodes.back();    
    };

	//lambda to categorize into closed and open based on their shape formed
    auto process = [&]( std::vector<int> &ways_nums ) {
        ...
        while( !open.empty() ) {            
            ...
            m_Ways.emplace_back(new_way); //creating new closed way
        }        
        std::swap(ways_nums, closed);	//update multiploygon with updated closed shape      
    };
	//constructing multipolygon processing both inner and outer boundaries 
    process(mp.outer);
    process(mp.inner);
}
